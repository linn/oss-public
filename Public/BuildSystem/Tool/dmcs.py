...
csccom = "$CSC4 $CSCFLAGS -t:$CSCTARGET -out:${TARGET.abspath} $SOURCES $_CSCLIBPATH $_CSCLIBS $_CSCPKGS $_CSCRESOURCES $_CSCWINICON"
csclibcom = "$CSC4 -t:library $CSCLIBFLAGS -out:${TARGET.abspath} $SOURCES $_CSCLIBPATH $_CSCLIBS $_CSCPKGS $_CSCRESOURCES $_CSCWINICON"

CsBuilder = SCons.Builder.Builder(action = '$CSCCOM4',
                                  source_factory = SCons.Node.FS.default_fs.Entry,
                                  prefix = '$CLIPROGPREFIX',
                                  suffix = '$CLIPROGSUFFIX',
                                  target_scanner = CsProgramScanner)

CsLibBuilder = SCons.Builder.Builder(action = '$CSCLIBCOM4',
                                     source_factory = SCons.Node.FS.default_fs.Entry,
                                     prefix = '$CLILIBPREFIX',
                                     suffix = '$CLILIBSUFFIX',
                                     target_scanner = CsProgramScanner)
...
def generate(env):
    ...
    env['CSCCOM4']         = SCons.Action.Action(csccom)
    env['CSCLIBCOM4']      = SCons.Action.Action(csclibcom)
    env['CSCTARGET']      = 'exe'
