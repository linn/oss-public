...
namespace KinskyDroid
{
        ...
        private void EnsureCacheItems(EventArgsItemsLoaded<upnpObject> e)
        {
            lock (iLevel2Cache)
            {
                ...
                for (int i = 0; i < e.Items.Count; i++)
                {
                    ...
                    {
                        BrowserItem item = new BrowserItem(e.Items[i], iIconResolver, iContainer.Metadata is musicAlbum, false);
                        if (!iFlingStateManager.IsFlinging())
                        {
                            item.EnsureLoaded();
            ...
            iInvoker.BeginInvoke((Action)(() =>
            {
                if (sender == iContentCollector)
                {
                    ...
                    if (iContainer.Metadata is musicAlbum)
                    {
                        ...
                        sectionHeaders.Add(new SectionHeader<BrowserItem>(0, new BrowserItem(iContainer.Metadata, iIconResolver, true, true)));
        ...
        public BrowserItem Item(int aIndex)
        {
            ...
            lock (iLevel2Cache)
            {
                if (!iLevel2Cache.TryGet(aIndex, out result) && !iFlingStateManager.IsFlinging())
                {
                    ...
                    if (item != null)
                    {
                        result = new BrowserItem(item, iIconResolver, iContainer.Metadata is musicAlbum, false);
    ...
    public class BrowserListAdaptor : AsyncArrayAdapter<BrowserItem, BrowserItem>
    {
        ...
        private void PopulateView(BrowserItem aItem, ViewCache aViewCache)
        {
            if (aItem != null)
            {
                ...
                secondLine.Text = aItem.DisplayField2;
                secondLine.Visibility = isGroupedItem ? ViewStates.Gone : ViewStates.Visible;
                TextView thirdLine = aViewCache.FindViewById<TextView>(Resource.Id.browseritemthirdline);
                thirdLine.Text = aItem.DisplayField3;
                thirdLine.Visibility = isGroupedItem && aItem.DisplayField3 == iHeaderArtist ? ViewStates.Gone : ViewStates.Visible;
    ...
    {
        public BrowserItem(upnpObject aItem, IconResolver aIconResolver, bool aIsGrouped, bool aIsGroupHeader)
        {
            iLock = new object();
            iItem = aItem;
