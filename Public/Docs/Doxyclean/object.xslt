...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="prototype">
		<xsl:choose>
			...
			<xsl:when test="@kind='event'">@event <xsl:apply-templates select="type"/>
				<xsl:choose>
					<xsl:when test="substring(type, string-length(type)-1, 2)=' *'"></xsl:when>
					<xsl:otherwise>
						<xsl:text> </xsl:text>
			...
			<xsl:when test="@kind='function'">
				...
				<xsl:choose>
				    <xsl:when test="contains(name/text(),':')">(<xsl:apply-templates select="type"/>)<xsl:call-template name="prototypeWithArguments"><xsl:with-param name="string" select="name"/></xsl:call-template></xsl:when>
				    <xsl:otherwise><xsl:apply-templates select="type"/><xsl:text> </xsl:text><xsl:value-of select="name"/><xsl:value-of select="argsstring"/></xsl:otherwise>
				</xsl:choose>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="compoundname">
		...
		<xsl:choose>
			<xsl:when test="../@kind='protocol'">
				<xsl:value-of select="substring-after(substring(.,0,string-length(.)-1), '::')"/>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="sectiondef[@kind='user-defined']">
		<section>
			...
			<xsl:apply-templates select="memberdef[(@prot!='private' and not (@kind='event' and @prot='protected')) and (briefdescription or detaileddescription)]"/>
		</section>
	</xsl:template>
	
	<xsl:template match="sectiondef[@kind!='user-defined']">
		<xsl:if test="count(memberdef[((@kind!='variable' and @prot!='private') and not (@kind='event' and @prot='protected')) and (briefdescription or detaileddescription)]) > 0">
			<section>
				<name>Other</name>
				<xsl:apply-templates select="memberdef[((@kind!='variable' and @prot!='private') and not(@kind='event' and @prot='protected')) and (briefdescription or detaileddescription)]"/>
			</section>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="memberdef">
		<member>
			<xsl:choose>
				<xsl:when test="@kind='function' and @static='yes' and @const='no' and @prot!='private'">
					<xsl:attribute name="kind">class-method</xsl:attribute>
				</xsl:when>
				<xsl:when test="@kind='function' and @static='no' and @const='no' and @prot!='private'">
					<xsl:attribute name="kind">instance-method</xsl:attribute>
				</xsl:when>
				<xsl:when test="@kind='property' and @prot!='private'">
					<xsl:attribute name="kind">property</xsl:attribute>
				</xsl:when>
				<xsl:when test="@kind='event' and @prot!='private' and @prot!='protected'">
					<xsl:attribute name="kind">event</xsl:attribute>
				</xsl:when>
			</xsl:choose>
			<xsl:if test="../../@kind='protocol' or ../../@kind='interface'">			
				<xsl:choose>
