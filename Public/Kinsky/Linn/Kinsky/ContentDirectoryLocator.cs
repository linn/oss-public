...
namespace Linn.Kinsky
{
    public class Location
    {
        ...
        public Location CommonAncestor(Location aOtherLocation)
        {
            List<IContainer> result = new List<IContainer>();
            for (int i = 0; i < this.Containers.Count && i < aOtherLocation.Containers.Count; i++)
            {
                if (this.Containers[i].Metadata.Id == aOtherLocation.Containers[i].Metadata.Id)
                {
                    result.Add(this.Containers[i]);
                }
                else
                {
                    break;
