...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:date="http://exslt.org/dates-and-times" version="1.0" extension-element-prefixes="date">
	...
	<xsl:template match="/">
		...
		<head>
			...
			<script type="text/javascript">
				...
				function toggleTOCItem(e) {
					var listItem = e.srcElement ? e.srcElement : e.target;
		...
		<body>
			...
			<nav id="buttons">
				<ul>
					<li id="toc_button"><button id="table_of_contents" onclick="toggleTOC()" class="open">Table of Contents</button></li>
					<li id="jumpto_button">
						<select id="jumpto" onchange="jumpTo()">
							...
							<xsl:call-template name="jumpEvents"/>
			...
			<nav id="tableOfContents" style="display: block;">
				<ul>
	...
	<xsl:template name="jumpEvents">
		<xsl:if test="count(object/sections/section/member[@kind='event']) > 0">
			<option value="properties">Events</option>
			<xsl:apply-templates select="object/sections/section/member[@kind='event']" mode="jumpList"/>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="object/sections" mode="toc">
		<xsl:if test="count(section) > 0">
			<li class="expandable" id="tocTasks" onclick="toggleTOCItem(event);">
	...
	<xsl:template name="TOCproperties">
		<xsl:if test="count(object/sections/section/member[@kind='property']) > 0">
			<li class="expandable" id="tocProperties" onclick="toggleTOCItem(event);">
	...
	<xsl:template name="TOCevents">
		<xsl:if test="count(object/sections/section/member[@kind='event']) > 0">
			<li class="expandable" id="tocEvents" onclick="toggleTOCItem(event);">
				<a href="#events">Events</a>
				<ul style="display: none;">
					<xsl:apply-templates select="object/sections/section/member[@kind='event']" mode="toc"/>
				</ul>
			</li>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="TOCclassMethods">
		<xsl:if test="count(object/sections/section/member[@kind='class-method']) > 0">
			<li class="expandable" id="tocClassMethods" onclick="toggleTOCItem(event);">
	...
	<xsl:template name="TOCinstanceMethods">
		<xsl:if test="count(object/sections/section/member[@kind='instance-method']) > 0">
			<li class="expandable" id="tocInstanceMethods" onclick="toggleTOCItem(event);">
	...
	<xsl:template match="object" mode="title">
		...
		<xsl:choose>
			...
			<xsl:when test="@kind='interface'"><xsl:text> Interface</xsl:text></xsl:when>
		</xsl:choose>
			...
			<span class="tooltipRegion">
				...
				<xsl:choose>
					...
					<xsl:when test="@kind='event'">
						<xsl:text> </xsl:text><span class="specialType">event</span>
					</xsl:when>
	...
	<xsl:template name="events">
		<xsl:if test="count(object/sections/section/member[@kind='event']) > 0">
			<section id="events">
				<h2>Events</h2>
				<xsl:apply-templates select="object/sections/section/member[@kind='event']" mode="details"/>
			</section>
		</xsl:if>
	</xsl:template>
